...
async function replan(value: string) {
  if (!plan.value) return
  const result = await plansStore.replanCompleted(plan.value.id, value)
  if (!result.ok) {
...
<template>
  <BasePage>
    ...
    <view v-if="plan?.status === 'completed'" class="detail">
      <view class="header"
        ><text class="header__date">{{ formatZhDate(plan.date || '') }}</text
        ><text v-if="plan.note" class="header__note">{{ plan.note }}</text></view
      >
      <view class="periods"
        ><PlanPeriodSection
      ...
      <view class="memories"
        ...
        ><scroll-view v-if="plan.memoryPhotos.length" scroll-x class="photos"
          ><image
            v-for="(photo, index) in plan.memoryPhotos"
            :key="`${photo}-${index}`"
            class="photo"
            :src="photo"
            mode="aspectFill" /></scroll-view
        ><text v-else class="empty">那天还没有留下打卡照片</text></view
      >
      <view class="records"
        ><text class="section-title">吃过和去过</text
        ><view v-if="placeRecords.length" class="record-group"
          ><text class="record-group__label">去过的地方</text
          ><text class="record-group__text">{{
            placeRecords.map((item) => item.title).join(' · ')
          }}</text></view
        ><view v-if="foodRecords.length" class="record-group"
          ><text class="record-group__label">吃过的味道</text
          ><text class="record-group__text">{{
            foodRecords.map((item) => item.title).join(' · ')
          }}</text></view
        ><text v-if="!placeRecords.length && !foodRecords.length" class="empty"
          >那天还没有安排记录</text
        ></view
      >
    </view>
    <view v-else class="missing"><text>没有找到这条过去的计划</text></view>
    <view v-if="plan?.status === 'completed'" class="footer"
      ><view class="primary" @tap="requestReplan"><text>再计划一次</text></view></view
    >
    <DatePickerSheet
      v-model="showDate"
      :value="date"
...
.record-group {
  ...
  gap: 8rpx;
  padding: 14rpx 0;
  border-bottom: 1rpx solid #e2d8d0;
}
.record-group__label {
  font-size: 23rpx;
  color: #a08474;
